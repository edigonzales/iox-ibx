...
public class IbxWriterTest {
  ...
  @Test
  public void directlyWritesEventsWithAllCompressions() throws Exception {
    for (String compression : Arrays.asList("none", "deflate", "zstd")) {
      ...
      Xtf24Reader r = new Xtf24Reader(new File("src/test/resources/tiny.xtf"));
      r.setModel(b.model);
      try (IbxWriter w = new IbxWriter(target, b.model, o)) {
        ...
        while ((e = r.read()) != null) {
          if (e instanceof ObjectEvent)
            ((ObjectEvent) e)
                .getIomObject()
                .setattrvalue(ch.interlis.iom_j.Iom_jObject.INTERNAL_T_ID, "123");
          w.write(e);
      ...
      try (IbxContainer c = IbxContainer.open(target);
          Fragment f = c.getTopic("Tiny.Data")) {
        assertEquals(3, f.baskets().count());
        ...
        assertTrue(
            f.objects()
                .allMatch(
                    o2 ->
                        o2.getObject().getattrvalue(ch.interlis.iom_j.Iom_jObject.INTERNAL_T_ID)
                            == null));
      }
    }
  }
  ...
  @Test
  public void incompleteOrInvalidTransferNeverPublishes() throws Exception {
    ...
    for (int failure = 0; failure < 4; failure++) {
      ...
      try (IbxWriter w = new IbxWriter(p, td, new WriterOptions())) {
        if (failure == 0) {
          w.write(new ch.interlis.iox_j.StartTransferEvent());
          w.flush();
          ...
          try {
            ...
            w.write(
                failure == 2
                    ? new ch.interlis.iox_j.StartTransferEvent()
  ...
  @Test
  public void existingDestinationSurvivesAbortAndOverwriteIsExplicit() throws Exception {
    TransferDescription td = bridge().model;
    ...
    WriterOptions overwrite = new WriterOptions();
    overwrite.overwrite = true;
    try (IbxWriter w = new IbxWriter(p, td, overwrite)) {
      w.write(new ch.interlis.iox_j.StartTransferEvent());
    }
  ...
  public void xtf23RoundtripRetainsVersionAndSemantics() throws Exception {
    ...
    TransferDescription td = ch.interlis.ili2c.Main.runCompiler(cfg);
    assertNotNull(td);
    Path input = tmp.newFile("input.xtf").toPath();
    // Independently encode the existing rich event fixture using iox-ili's 2.3 writer.
    Xtf24Reader r = new Xtf24Reader(new File("src/test/resources/tiny.xtf"));
    r.setModel(bridge().model);
    XtfWriter out = new XtfWriter(input.toFile(), td);
    try {
      IoxEvent e;
      while ((e = r.read()) != null) {
        if (e instanceof StartTransferEvent)
          e = new ch.interlis.iox_j.StartTransferEvent("test sender", "original comment", "2.3");
        if (e instanceof ObjectEvent) {
          ch.interlis.iom.IomObject object = ((ObjectEvent) e).getIomObject();
          if ("o1".equals(object.getobjectoid())) {
            object.setattrvalue(
                "xml",
                "<q:payload xmlns:q=\"urn:ibx-test\"><q:text>A &amp; B</q:text></q:payload>");
            object.setattrvalue("binary", "AAEC");
          }
        }
        out.write(e);
        if (e instanceof ObjectEvent
            && "o2".equals(((ObjectEvent) e).getIomObject().getobjectoid())) {
          ch.interlis.iom.IomObject link =
              new ch.interlis.iom_j.Iom_jObject("Tiny.Data.Link", null);
          link.addattrobj("a", "REF").setobjectrefoid("o1");
          link.addattrobj("b", "REF").setobjectrefoid("o2");
          out.write(new ch.interlis.iox_j.ObjectEvent(link));
        }
        if (e instanceof EndTransferEvent) break;
      }
    } finally {
    ...
    WriterOptions o = new WriterOptions();
    o.modelFiles.add(ili.toString());
    Path ibx = tmp.getRoot().toPath().resolve("data.ibx");
    ContainerWriter.create(input, ibx, o);
    Path roundtrip = tmp.newFile("roundtrip.xtf").toPath();
    try (IbxContainer c = IbxContainer.open(ibx);
        OutputStream output = Files.newOutputStream(roundtrip)) {
      assertEquals("2.3", c.metadata().version);
      c.export(output);
    }
    ModelBridge b = ModelBridge.load(input, o, tmp.newFolder().toPath());
    ch.interlis.ibx.benchmark.RoundtripVerifier.verify(
        input, roundtrip, b, tmp.newFolder().toPath());
    Path again = tmp.getRoot().toPath().resolve("again.ibx");
    ContainerWriter.create(roundtrip, again, o);
  }
}
