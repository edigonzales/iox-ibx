...
public final class IbxWriter implements IoxWriter, AutoCloseable {
  ...
  public IbxWriter(File target, TransferDescription model, WriterOptions options) throws IoxException {
    this(target.toPath(), model, options);
  }
  public IbxWriter(Path target, TransferDescription model, WriterOptions options) throws IoxException {
    if (target == null || model == null || options == null) throw new IoxException("Target, model and options are required");
    this.target = target; this.model = model; this.options = options;
    try { options.validate(); } catch (RuntimeException e) { throw new IoxException(e); }
  }
  ...
  public IbxWriter(Path target, ModelBridge bridge, WriterOptions options) throws IoxException {
    this(target, bridge.model, options); this.bridge = bridge;
  }
  ...
  @Override public void write(IoxEvent event) throws IoxException {
    try {
      ...
      if (event instanceof StartTransferEvent) {
        ...
        if (bridge == null) {
          ...
          if (models != null) { meta.transferModels.clear(); meta.transferModels.addAll(models); }
          if (version == null || meta.transferModels.isEmpty()) throw new IoxException("No transfer models selected");
          meta.version = version;
          Path temp = options.temporaryDirectory == null ? Paths.get(System.getProperty("java.io.tmpdir")) : options.temporaryDirectory;
        ...
        if (event instanceof EndTransferEvent) { ended = true; session.close(); }
      }
    } catch (Exception e) {
      try { close(); } catch (IoxException cleanup) { e.addSuppressed(cleanup); }
      throw e instanceof IoxException ? (IoxException)e : new IoxException(e);
    }
  }
  @Override public void flush() throws IoxException {
    if (closed) throw new IoxException("Writer is closed");
  }
  @Override public void close() throws IoxException {
    if (closed) return;
    closed = true;
    if (session != null) try { session.close(); } catch (Exception e) { throw new IoxException(e); }
  }
  @Override public IoxFactoryCollection getFactory() { return factory; }
  @Override public void setFactory(IoxFactoryCollection factory) throws IoxException {
    if (factory == null) throw new IoxException("Factory is required");
    this.factory = factory;
  }
  @Override public IomObject createIomObject(String tag, String oid) throws IoxException {
